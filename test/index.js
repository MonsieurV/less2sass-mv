...
describe("less2sass", function () {
  describe("whole file", function () {
    ...
    before(function () {
      fixture = fs
        .readFileSync(path.resolve(__dirname, "fixtures/test.less"))
        .toString();
      expected = fs
        .readFileSync(path.resolve(__dirname, "fixtures/test.scss"))
        .toString();
    });
  ...
  describe("variables", function () {
    it("converts interpolated variables to #{$", function () {
      const result = less2sass.convert(
        "@san-serif-stack: helvetica, arial; @standard-fonts: ~'@{san-serif-stack}, sans-serif';"
      );
      assert.equal(
        result,
        "$san-serif-stack: helvetica, arial; $standard-fonts: '#{$san-serif-stack}, sans-serif';"
